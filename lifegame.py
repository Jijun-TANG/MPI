...
class grille:
    ...
    def __init__( self, dim, init_pattern = None, color_life = "black", color_dead = "white" ):
        ...
        if init_pattern is not None:
            ...
            print("pattern initial est None")
            self.cells = np.random.randint(2, size=dim, dtype=np.uint8)
        self.col_life = color_life
        self.col_dead = color_dead

    def compute_next_iteration_para(self,p,q,Nx):
        """
        Calcule la prochaine génération de cellules en suivant les règles du jeu de la vie pour un rank en parallèle
        """
        ...
        tx=nx-2
        ty=ny-2
        def coordinate_global(i:int,j:int)->int:
            ind=(p*tx+i-1)*Nx+q*ty+j-1
            return ind
        diff_cells = []
            ...
            for j in range(1,nx-1):
                ...
                if self.cells[i,j] == 1: # Si la cellule est vivante
                    if (nb_voisines_vivantes < 2) or (nb_voisines_vivantes > 3):
                        ...
                        ig=coordinate_global(i,j)
                        diff_cells.append(ig)
                    else:
